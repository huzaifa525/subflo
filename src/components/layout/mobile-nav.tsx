    ...
    <>
      {moreOpen && (
        <div className="fixed inset-0 z-40 md:hidden" onClick={() => setMoreOpen(false)}>
          <div className="absolute bottom-14 right-3 sf-card shadow-xl py-1 w-40" onClick={(e) => e.stopPropagation()}>
            {moreLinks.map((l) => (
              <Link
                key={l.href}
                href={l.href}
                ...
                style={{ color: pathname.startsWith(l.href) ? "var(--accent)" : "var(--text-secondary)" }}
              >
                {l.label}
              </Link>
            ))}
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden border-t" style={{ background: "var(--bg-secondary)", borderColor: "var(--border-default)" }}>
        <div className="flex items-center justify-around py-1.5">
          ...
          <button
            ...
            style={{ color: moreOpen || ["/calendar", "/inbox", "/import", "/family", "/settings"].some((p) => pathname.startsWith(p)) ? "var(--accent)" : "var(--text-tertiary)" }}
          >
            <svg width="18" height="18" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="4" r="1.5" fill="currentColor"/><circle cx="8" cy="8" r="1.5" fill="currentColor"/><circle cx="8" cy="12" r="1.5" fill="currentColor"/></svg>
